class Prob4_Part1:
	...
	"""
		...
		try:
			...
			if len(hint)>0:
                                return hint+' If there are 2% colorblind males, then what is the probability that a person chosen at random is colorblind if we know that the person is male?','2/100'
                        else:
                                return '',''
		except Exception:
		        return '',''
